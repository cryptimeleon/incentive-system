...
public class BasketController {
  ...
  /**
   * Adds #count many items with the matching itemId to the basket if not present. Overwrites the
   * count if present.
   */
  ...
  void putItem(@RequestBody PutItemRequest putItemRequest) throws BasketServiceException {
    ...
    basketService.setItemInBasket(
        putItemRequest.getBasketId(), putItemRequest.getItemId(), putItemRequest.getCount());
  }

  /** Removes an item from the basket. */
  @DeleteMapping("/basket/items")
  void deleteItem(@RequestHeader UUID basketId, @RequestParam UUID itemId)
      throws BasketServiceException {
    basketService.deleteItemFromBasket(basketId, itemId);
  }

  /**
   * Sets a basket to paid. TODO: shared secret to prevent users from doing this. Send 403 to normal
   * users TODO add hashcode for integrity?
   */
  ...
  /**
   * Sets a basket to redeemed and stores the redeem request. The redeem amount is modelled as
   * basket's value Returns an error code if the basket cannot be redeemed. The same request can be
   * used multiple times to allow users to recover from network errors etc.
   *
   * <p>TODO: shared secret to prevent users from doing this. Send 403 to normal users
   */
  @PostMapping("/basket/redeem")
  void redeemBasket(@RequestBody RedeemBasketRequest redeemRequest) throws BasketServiceException {
    basketService.redeemBasket(
        redeemRequest.getBasketId(), redeemRequest.getRedeemRequest(), redeemRequest.getValue());
  }
  ...
  public ResponseEntity<String> handleBasketServiceException(
      BasketServiceException basketServiceException) {
