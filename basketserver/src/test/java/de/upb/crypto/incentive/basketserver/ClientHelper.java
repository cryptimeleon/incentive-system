...
public class ClientHelper {
  ...
  static EntityExchangeResult<Basket> queryBasket(
      WebTestClient webClient, UUID basketId, HttpStatus expectedStatus) {
    return webClient
        .get()
        .uri("/basket")
  ...
  static void putItem(
      WebTestClient webTestClient, PutItemRequest putItemRequest, HttpStatus expectedStatus) {
    webTestClient
        .put()
        .uri("/basket/items")
  ...
  static void deleteBasketItem(
      WebTestClient webTestClient, UUID basketId, UUID itemId, HttpStatus expectedStatus) {
    webTestClient
        .delete()
        .uri(uriBuilder -> uriBuilder.path("/basket/items").queryParam("itemId", itemId).build())
        .header("basketId", String.valueOf(basketId))
        .exchange()
        .expectStatus()
        .isEqualTo(expectedStatus);
  ...
  static void payBasket(
      WebTestClient webTestClient, UUID basketId, long value, HttpStatus expectedStatus) {
    var payRequest = new PayBasketRequest(basketId, value);
    payBasket(webTestClient, payRequest, expectedStatus);
  }

  static void payBasket(
      WebTestClient webTestClient, PayBasketRequest payBasketRequest, HttpStatus expectedStatus) {
    webTestClient
        .post()
        .uri("/basket/pay")
  ...
  static void redeemBasket(
      WebTestClient webTestClient, RedeemBasketRequest redeemRequest, HttpStatus expectedStatus) {
    webTestClient
        .post()
        .uri("/basket/redeem")
