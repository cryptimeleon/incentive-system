package org.cryptimeleon.incentive.services.dsprotection.mock;

import org.cryptimeleon.incentive.services.dsprotection.storage.DsIdEntry;
...
public class MockDsidEntryRepository implements DsidRepository {
    ...
    public MockDsidEntryRepository() {
        doubleSpendingIdEntries = new HashMap<>();
        nextId = 1;
    }
    ...
    public <S extends DsIdEntry> Iterable<S> saveAll(Iterable<S> dsidEntries) {
        for (DsIdEntry dside : dsidEntries) {
            doubleSpendingIdEntries.put(nextId, dside);
            // Note: IDs are wrongly autogenerated here
            // (more precisely: always 0)
            // since we do not boot up H2 framework (which would generate incrementing IDs automatically) when using this mock repository
            // => we need to manually set the ID of the new entry to the respective hash map key
    ...
    public ArrayList<DsIdEntry> findAll() {
        ArrayList<DsIdEntry> results = new ArrayList<>();
    ...
    public ArrayList<DsIdEntry> findAllById(Iterable<Long> ids) {
        ...
        for (long id : ids) {
            Optional<DsIdEntry> dside = findById(id);
            dside.ifPresent(results::add);
        }
    ...
    public void deleteAll() {
        this.doubleSpendingIdEntries = new HashMap<>();
    }
}
