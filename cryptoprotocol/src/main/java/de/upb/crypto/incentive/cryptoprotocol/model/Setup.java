...
/**
 * a class implementing the incentive system algorithms (of the incentive system from the 2020
 * paper) that are not part of any protocol (namely Setup, P.KeyGen, U.KeyGen)
 */
public class Setup {
  ...
  public static PublicParameters trustedSetup(int securityParameter) {
    // generate a bilinear group from the security parameter (type 3,
    // Barreto-Naehrig)
    BilinearGroup bg = new BarretoNaehrigBilinearGroup(securityParameter);

    // TODO: rewrite computation of w and h7 once proper hashing of bilinear groups
    // is possible
    // compute w (base used in double spending protection, see 2020 incsys paper)
    GroupElement w =
        bg.getG1().getUniformlyRandomElement(); // w=e not a problem, see discord #questions 2.2.21

    // compute seventh base for user token (h7 in 2020 incsys paper)
    GroupElement h7 = bg.getG1().getUniformlyRandomElement();

    // instantiate prf used in this instance of the incentive system with a proper
    // key length
  ...
  public static UserKeyPair userKeyGen(PublicParameters pp) {
    ...
    Zn usedZn =
        pp.getBg()
            .getZn(); // the remainder class ring used in this instance of the incentive system
    ZnElement usk = usedZn.getUniformlyRandomElement(); // secret exponent

    // generate the user key for the PRF used to generate pseudorandomness in the
    // system
  ...
  public static ProviderKeyPair providerKeyGen(PublicParameters pp) {
    // draw the dlogs of the first 6 bases used in the Pedersen commitment in the
    // token
    ...
    SignatureKeyPair<SPSEQVerificationKey, SPSEQSigningKey> spsEqKeyPair =
        pp.getSpsEq().generateKeyPair(2);
