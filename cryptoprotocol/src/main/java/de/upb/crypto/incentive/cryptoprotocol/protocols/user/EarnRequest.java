...
/**
 * Class representing an earn request created by a user of the incentive system. Provides
 * functionality to initialize the request (i.e. preparing all data that needs to be send to the
 * provider on making the request) as well as handling the providers response. Note that not all
 * fields of this class are part of its serialized representation (for privacy reasons).
 */
public class EarnRequest implements EarnInterface, Representable {
  @Represented private PublicParameters pp;
  @Represented private ProviderPublicKey pk;
  private UserSecretKey usk;
  private long k; // TODO: does k even need
  private Token token; // TODO: is it a problem if token is unserialized? -> shouldn't be because
  // serialization is no encryption

  @Represented
  private GroupElement blindedCommitment; // the blinded commitment sent in the earn request

  @Represented private SPSEQSignature blindedCertificate;
  private ZnElement s; // blinding randomness, maybe easier to retry requests if the randomness is
  // saved in the
  // requests instance

  /** @param k earn amount, name taken from 2020 inc sys paper */
  public EarnRequest(
      PublicParameters pp, ProviderPublicKey pk, UserSecretKey usk, long k, Token token) {
    Zn usedZn = pp.getBg().getZn();
    // draw blinding value
    this.s =
        usedZn.getUniformlyRandomNonzeroElement(); // s cannot be zero since we need to compute its
    // inverse to unblind the signature
  ...
  public String generateSerializedEarnRequest() {
    ...
    SPSEQVerificationKey provSPSPk =
        this.pk.getPkSpsEq(); // store SPS EQ verification key from provider public key
    this.blindedCommitment =
        this.token.getToken().pow(s); // computing another representative with blinding
    // randomness (the implemented
    // SPS-EQ is over R_exp)
    this.blindedCertificate =
        (SPSEQSignature) pp.getSpsEq().chgRep(certificate, this.s, provSPSPk); // note: in
    // contrast
    // to formal
    // specification,
    // chgrep
    // only
    // takes
    // three arguments (no message) and thus only updates the signature
