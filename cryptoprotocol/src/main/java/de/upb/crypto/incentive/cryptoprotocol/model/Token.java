...
/**
 * data holding class representing a token from a mathematical point of view (meaning: as a bunch of
 * group elements and exponents). serialized representation of token does not contain secret
 * exponents and token plain text (latter can be computed trivially).
 */
public class Token implements Representable {
  @Represented
  private PublicParameters
      pp; // public parameters for the inc sys instance in which the token was created

  @Represented
  private GroupElement token; // the Pedersen commitment computed from the bases and the exponents,
  // representing the
  // actual token

  private GroupElementPlainText
      tokenPlainText; // plaintext version of token, needed for compliance with verification
  // method

  @Represented
  private SPSEQSignature
      certificate; // the SPS-EQ certifying the commitment as well-formed and valid

  @Represented
  private VerificationKey
      verificationKey; // the SPS-EQ pk used to check validity of token-certificate pair

  private GroupElementVector
      commitmentBases; // array of the 7 group elements (h_i) needed to form the Pedersen
  // commitment
  private ZnElement userSecretKey; // secret key of the user the token belongs to
  private ZnElement
      encryptionSecretKey; // secret key used for the ElGamal encryption in the Spend algorithm
  private ZnElement
      doubleSpendRandomness0; // randomness used for the first challenge generation in double
  // spending protection
  private ZnElement
      doubleSpendRandomness1; // randomness used for the second challenge generation in double
  // spending protection
  ...
  /**
   * standard constructor, intializes an empty token, meaning a token storing 0 points represented
   * as a Pedersen commitment. Note that certificate is not set since a token is usually not
   * certified upon creation.
   *
  ...
  public Token(
    ...
    Zn myRemainderClassRing =
        pp.getBg().getZn(); // needed to retrieve correct zero element to compute point count
   ...
   * (and also updates its plaintext representation) (see chapter 4 of 2020 incentive systems paper)
   */
  private void recomputeCommitment() {
    // note that indices of bases are off by one wrt paper (zero-based indexing)
    this.token =
        this.commitmentBases
            .get(0)
            .pow(this.userSecretKey)
  ...
  public void setCertificate(SPSEQSignature cert) throws SPSEQException {
    ...
    if (!signatureScheme.verify(
        this.tokenPlainText, this.certificate, this.verificationKey)) // token plaintext
    // needed for
    // verification (API
    // reasons)
    {
